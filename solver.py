...
# scheme subject to parameters such as budget constraint, etc.  This script does not consider
...
filename = sys.argv[1] # pass the name of CSV file generated by calculateValues.py (e.g. $ solver.py results.csv)
...
outputFilename = "graphDecommissioned.png"
...
while not done:
    ...
    for index in edgeIndices:
        ...
        if not gn.G.has_edge(u,v,0) and not gn.G.has_edge(u,v,1):
            ...
            df = df[(df['edge_u'] != u) & (df['edge_v'] != v)] # drop this row since edge is now gone
            break
        ...
        cost += getConversionCost(gn, gasPruned)
        ...
        cost += getOverloadCost(en, upgradedTrans)

        cost = math.ceil(cost/10) # adjust for knapsack (scaled by factor of 10)

        # if the cost turns out to be more than the remaining budget, need to discard this option
        if cost > runningBudget:
            # roll back
            gn.G = oldG
        ...
        runningBudget = runningBudget - cost
        print(runningBudget)

    # check whether we should terminate.
    if not done:
        # if our remaining budget is smaller than the cost of any options, we are done.
        if runningBudget < min(weightsQ):
            done = True
            continue
        ...
        # compute knapsack for the given list of items
        result = dpOptimalKnapsack(runningBudget, weightsQ, values, len(values))
        edgeIndices = result[1]
...
totalCost = budget - runningBudget
